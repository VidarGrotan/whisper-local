...
The tray may display the `Dictation` profile because `profiles.yaml` contains `active: dictation`. That label does not prove the profile overrides match the live configuration. Avoid selecting the shipped `Dictation` or `Notes` profiles until they are customized: both currently specify the smaller `base` model and can replace `large-v3-turbo`.
...
Last verified after dual-model routing: 178 tests passed, with 4 platform-specific skips. Synthetic installed-configuration checks confirmed:
